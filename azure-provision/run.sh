...
nohup az vm run-command invoke -g 'kafka-rg' -n 'database' --command-id RunShellScript --scripts @azure-provision/postinstall-scripts/install.sh &
...
echo "SQL Database Public IP: $(az vm show -d -g 'kafka-rg' -n kafka2 --query publicIps -o tsv), Access Port: 1433"