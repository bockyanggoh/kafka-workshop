...
namespace CAKafka.Library.impl
{
    public class KafkaSubscriber<T>: IKafkaSubscriber<T> where T : class
    {
        ...
        public KafkaSubscriber(KafkaOptions options)
        {
            _option = options;
            _consumerConfig = new ConsumerConfig
            {
                BootstrapServers = GenerateKafkaBrokerString(options),
            ...
            {
                Url = $"{options.Servers.SchemaRegistry.PublicIp}:{options.Servers.SchemaRegistry.Port}",
